        ...
        <>
            {alarmPopUp ? (
                ...
                <S.AlbumCard>
                    ...
                    <S.AlbumImage src={albumPhoto} height={121} width={121} alt="foto do àlbum" className="rounded-[4px] mx-4" draggable="false"></S.AlbumImage>
                    <S.AlbumInfo className="h-auto w-[282px] grid items-center content-center">
                        <Text $size="small" $weight="regular" color="white">In Bloom</Text>
                        <Text $size="small" $weight="regular" color="white"> NECKDEEP </Text>
                    </S.AlbumInfo>
                    ...
                    <S.ActionGridContainer className="grid justify-items-center items-end mr-4 h-[121px]">
                        ...
                        <S.Controls className="grid items-center justify-items-center h-11 w-7">
                            {isMusicAdded ? (
                                <>
                                    {isDone ? (
                                        <button><img src={escutado} height={28} width={28} alt="done icon" draggable="false" /></button>
                                    ) : (
                                        <button><img src={isAlarmSet ? alarmeAtivo : adicionarAlarme} height={28} width={28} alt="alarm or plus icon" draggable="false" /></button>
                                    )}
                                </>
                            ) : (
                                <S.AddButton onClick={handleAdd}></S.AddButton>
                            )}
                            <Text $size="xSmall" $weight="regular">3:14</Text>
                        </S.Controls>
